// ==WindhawkMod==
// @id              nbc-translucent-windows
// @name            亚克力窗口
// @description     为你的 windows 11 窗口主题加点料
// @version         1.6.3
// @author          Ni But Crazy
...
- RenderingMod:
    - ThemeBackground: FALSE
      $name: Windows 主题自定义渲染
      $description: >-
        使用 Direct2D 图形应用程序接口修改 Windows 主题的部分内容
    - AccentColorControls: FALSE
      $name: Windows 主题调色器
      $description: >-
       为 Windows 主题部分涂上强调色(需要 Windows 主题自定义渲染）
    - TextAlphaBlend: FALSE
      $name: 文字的透明度混合
      $description: >-
        对 Windows GDI 文本渲染进行 Alpha 混合
  $name: 自定义渲染
- type: none
  $name: 效果
  $description: >-
    要使用 SystemBackdrop 效果，需要 Windows 11 版本 ≥ 22621.xxx（22H2）
  $options:
  - none: Default
...
- AccentBlurBehind: "00000000"
  $name: AccentBlurBehind 背景虚化的颜色混合
  $description: >-
    将颜色与模糊后的背景进行混合
    颜色使用十六进制 ARGB 格式表示，例如 0F000000
- ImmersiveDarkTitle: FALSE
  $name: 自动切换深色模式标题栏（监听型）
  $description: >-
    正常来讲，一个新打开的应用会自行匹配深色模式标题栏，但像是explorer这种只打开新窗口而不重新启动的应用只能使用监听模式了
- ExtendFrame: FALSE
  $name: 将效果扩展到整个窗口
  $description: >-
    使用 DwmExtendFrameIntoClientArea 将效果扩展到整个窗口背景
- CornerOption: default
  $name: 窗口角类型
  $description: >-
    指定窗口的圆角偏好设置
    需要 Windows 11 版本 ≥ 22000.xxx（21H2）
  $options: 
  - default: Default
  - notrounded: 无圆角
  - smallround: win11 圆角
- RainbowSpeed: 1
  $name: 彩虹速度
  $description: 调整彩虹颜色的刷新速度（取值范围 1 到 100）
- TitlebarColor:
    - ColorTitlebar: FALSE
      $name: 开启
    - RainbowTitlebar: FALSE
      $name: 彩虹标题栏
      $description: 启用窗口标题栏的彩虹效果
    - titlerbarstyles_active: "FF0000"
      $name: 聚焦窗口颜色
      $description: >-
       以十六进制 RGB 格式表示的颜色，例如红色 = FF0000
        SystemAccentColor = 2
    - titlerbarstyles_inactive: "00FFFF"
      ...
      $description: >-
       以十六进制 RGB 格式表示的颜色，例如红色 = FF0000
        SystemAccentColor = 2
  $name: 标题栏颜色
  $description: 需要 Windows 11 版本 ≥ 22000.xxx（21H2），此设置将覆盖效果设置。
- TitlebarTextColor:
    - ColorTitlebarText: FALSE
      $name: 开启
    - RainbowTextColor: FALSE
      $name: 彩虹标题栏文本颜色
      $description: 启用窗口标题栏文字的彩虹效果
  ...
  $description: >-
     需要 Windows 11 版本 ≥ 22000.xxx（21H2）。
     注意：此设置仅影响传统 Win32 窗口。由于在 Windows 11 22H2 中，文件资源管理器的标题栏文字渲染方式已改为使用 DirectWrite API，因此要修改文件资源管理器标题栏的文字颜色，请使用 Windhawk 的 File Explorer Styler 模块
- BorderColor:
    - ColorBorder: FALSE
      $name: 开启
    - RainbowBorder: FALSE
      $name: 彩虹边框
      $description: 启用窗口边框的彩虹效果
    ...
    - MenuBorderColor: FALSE
      $name: 将彩色边框效果扩展到经典右键菜单
      $description: 启用此选项可在 Windows 经典右键菜单中显示彩色边框
  $name: 边框颜色
  $description: >-
      需要 Windows 11 版本 ≥ 22000.xxx（21H2）
- RuledPrograms:
...
static constexpr UINT SMALLROUND = 2; // DWMWCP_ROUNDSMALL
...
VOID LoadSettings(VOID)
{
    ...
    GetColorSetting(WindhawkUtils::StringSetting(Wh_GetStringSetting(L"AccentBlZurBehind")), g_settings.AccentBlurBehindClr);

    // g_settings.ImmersiveDarkmode = Wh_GetIntSetting(L"ImmersiveDarkTitle");
    g_settings.AutoImmersiveDark = Wh_GetIntSetting(L"ImmersiveDarkTitle");
    BOOL dark = IsSystemDarkMode();
    g_settings.ImmersiveDarkmode = dark;
    ...
    for (INT i = 0;; i++) 
    {
        ...
        if (hasProgram) 
        {
            ...
            if(currproc == ruledproc)
            {
                ...
                g_settings.AutoImmersiveDark = g_settings.ImmersiveDarkmode = Wh_GetIntSetting(L"RuledPrograms[%d].ImmersiveDarkTitle", i);
                g_settings.ImmersiveDarkmode = dark;
...
LRESULT CALLBACK HiddenWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (msg == WM_SETTINGCHANGE) {
        g_settings.ImmersiveDarkmode = IsSystemDarkMode();
        ApplyForExistingWindows();
        // TODO: 调用 DwmSetWindowAttribute 更新标题栏等效果
    }
...
//     return wcsstr(path, L"explorer.exe") != nullptr;
// }


void CreateThemeMonitorWindow(HINSTANCE hInst) {
    const wchar_t* kThemeMonitorClassName = L"ThemeMonitorHiddenWnd_GlobalSingleton";
    HWND hWndExisting = FindWindow(kThemeMonitorClassName, nullptr);
    if (hWndExisting) {
        // 已经有别的进程创建了监听窗口
        Wh_Log(L"监听器窗口已存在，跳过创建");
        return;
    }
    WNDCLASS wc = {0};
    wc.lpfnWndProc = HiddenWndProc;
    wc.hInstance = hInst;
    wc.lpszClassName = L"ThemeMonitorHiddenWnd_GlobalSingleton";
    RegisterClass(&wc);

    g_hHiddenWnd = CreateWindowEx(0, wc.lpszClassName, L"",
        WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
        CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInst, NULL);

    ShowWindow(g_hHiddenWnd, SW_HIDE); // 隐藏窗口
    Wh_Log(L"系统主题监听器已启动");
}
...
BOOL Wh_ModInit(VOID) 
{
    ...
    if (g_settings.AutoImmersiveDark) {
        CreateThemeMonitorWindow(GetModuleHandle(nullptr));
    }
